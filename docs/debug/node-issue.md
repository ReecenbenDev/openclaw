...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Reecenbot
...
Running Reecenbot via Node with `tsx` fails at startup with:

```
[reecenbot] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `reecenbot status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch reecenbot.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node reecenbot.mjs status` works on Node 25.
