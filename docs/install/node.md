...
read_when:
  - "You installed Reecenbot but `reecenbot` is “command not found”"
...
Reecenbot’s runtime baseline is **Node 22+**.

If you can run `npm install -g reecenbot@latest` but later see `reecenbot: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
