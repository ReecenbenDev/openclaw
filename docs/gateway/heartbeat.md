...
- During heartbeat runs, Reecenbot treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `reecenbot sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Reecenbot skips the heartbeat run entirely (no model call).
...
```bash
reecenbot system event --text "Check for urgent follow-ups" --mode now
```
