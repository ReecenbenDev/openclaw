...
- Replaces the legacy `gateway` command. CLI entry point: `reecenbot gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)

```bash
reecenbot gateway --port 18789
# for full debug/trace logs in stdio:
reecenbot gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
reecenbot gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```

- Config hot reload watches `~/.reecenbot/reecenbot.json` (or `REECENBOT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__reecenbot__/canvas/` from `~/.reecenbot/workspace/canvas`. Disable with `canvasHost.enabled=false` or `REECENBOT_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `REECENBOT_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `REECENBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.reecenbot.*` may still exist)
- Linux: `reecenbot-gateway-<profile>.service`
- Windows: `Reecenbot Gateway (<profile>)`

Install metadata is embedded in the service config:

- `REECENBOT_SERVICE_MARKER=reecenbot`
- `REECENBOT_SERVICE_KIND=gateway`
- `REECENBOT_SERVICE_VERSION=<version>`
...
```bash
reecenbot --dev setup
reecenbot --dev gateway --allow-unconfigured
# then target the dev instance:
reecenbot --dev status
reecenbot --dev health
```

Defaults (can be overridden via env/flags/config):

- `REECENBOT_STATE_DIR=~/.reecenbot-dev`
- `REECENBOT_CONFIG_PATH=~/.reecenbot-dev/reecenbot.json`
- `REECENBOT_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.reecenbot/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `REECENBOT_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `REECENBOT_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `REECENBOT_CONFIG_PATH`
- unique `REECENBOT_STATE_DIR`
...
```bash
reecenbot --profile main gateway install
reecenbot --profile rescue gateway install
```

Example:

```bash
REECENBOT_CONFIG_PATH=~/.reecenbot/a.json REECENBOT_STATE_DIR=~/.reecenbot-a reecenbot gateway --port 19001
REECENBOT_CONFIG_PATH=~/.reecenbot/b.json REECENBOT_STATE_DIR=~/.reecenbot-b reecenbot gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `reecenbot health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `reecenbot`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `reecenbot gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.reecenbot.*` is cleaned up).
  - `reecenbot doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Reecenbot profile services.
  ...
  - Cleanup: `reecenbot gateway uninstall` (current service) and `reecenbot doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `reecenbot gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- Reecenbot.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.reecenbot.*` labels still unload cleanly).
- To stop it cleanly, use `reecenbot gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `reecenbot gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `reecenbot gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

Reecenbot installs a **systemd user service** by default on Linux/WSL2. We
...
`reecenbot gateway install` writes the user unit. `reecenbot doctor` audits the
...
Restart=always
RestartSec=5
Environment=REECENBOT_GATEWAY_TOKEN=
...
Create `/etc/systemd/system/reecenbot-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now reecenbot-gateway[-<profile>].service
```
...
- `reecenbot gateway health|status` — request health/status over the Gateway WS.
- `reecenbot message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `reecenbot agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `reecenbot gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `reecenbot gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `reecenbot gateway` and the legacy TCP control port.
