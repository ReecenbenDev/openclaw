...
`reecenbot doctor` is the repair + migration tool for Reecenbot. It fixes stale
...
- Extra workspace dir detection (`~/reecenbot`).
...
you to run `reecenbot doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.reecenbot/reecenbot.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.reecenbot/sessions/` to `~/.reecenbot/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.reecenbot/agent/` to `~/.reecenbot/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.reecenbot/credentials/*.json` (except `oauth.json`)
  - to `~/.reecenbot/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `reecenbot doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.reecenbot` folders exist across
  home directories or when `REECENBOT_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.reecenbot/reecenbot.json` is
...
offers to remove them and install the Reecenbot service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Reecenbot gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `reecenbot doctor --generate-gateway-token` to force token
...
- `reecenbot doctor` prompts before rewriting supervisor config.
- `reecenbot doctor --yes` accepts the default repair prompts.
- `reecenbot doctor --repair` applies recommended fixes without prompts.
- `reecenbot doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `reecenbot gateway install --force`.
