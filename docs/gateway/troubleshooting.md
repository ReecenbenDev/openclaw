---
summary: "Quick troubleshooting guide for common Reecenbot failures"
read_when:
...
When Reecenbot misbehaves, here's how to fix it.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `reecenbot status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `reecenbot status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `reecenbot status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `reecenbot gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `reecenbot channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `reecenbot gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `reecenbot logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `reecenbot status --all` (it redacts tokens). If you paste `reecenbot status`, consider setting `REECENBOT_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  reecenbot models auth setup-token --provider anthropic
  ```
...
```bash
reecenbot models auth paste-token --provider anthropic
reecenbot models status
```
...
- Preferred: `reecenbot logs --follow`
- File logs (always): `/tmp/reecenbot/reecenbot-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$REECENBOT_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u reecenbot-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  reecenbot config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  reecenbot config set gateway.mode remote
  reecenbot config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `reecenbot setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.reecenbot/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `reecenbot doctor`
...
- then run `reecenbot sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `reecenbot gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `reecenbot gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `reecenbot gateway status` and `reecenbot doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `REECENBOT_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `reecenbot gateway status` shows a config mismatch**
...
- Fix: rerun `reecenbot gateway install --force` from the same `--profile` / `REECENBOT_STATE_DIR` you want the service to use.

**If `reecenbot gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `reecenbot doctor` to update it (or `reecenbot gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `REECENBOT_GATEWAY_TOKEN`) and restart the service.

**If `reecenbot gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/reecenbot` on disk.
...
Symptoms: `pwd` or file tools show `~/.reecenbot/sandboxes/...` even though you
...
Reecenbot intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `reecenbot models list` or
  `reecenbot models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${REECENBOT_CONFIG_PATH:-$HOME/.reecenbot/reecenbot.json}"
```
...
tail -f "$(ls -t /tmp/reecenbot/reecenbot-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
reecenbot pairing list <channel>
```
...
```bash
reecenbot logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.reecenbot/agents/<agentId>/sessions/
```
...
reecenbot logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
reecenbot gateway --verbose
```

If you’re logged out / unlinked:

```bash
reecenbot channels logout
trash "${REECENBOT_STATE_DIR:-$HOME/.reecenbot}/credentials" # if logout can't cleanly remove everything
reecenbot channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/reecenbot/reecenbot-*.log | head -1)" | tail -20
```

### High Memory Usage

Reecenbot keeps conversation history in memory.
...
Reecenbot now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
```bash
reecenbot doctor
reecenbot doctor --fix
```

Notes:

- `reecenbot doctor` reports every invalid entry.
- `reecenbot doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `reecenbot logs`, `reecenbot health`, `reecenbot status`, `reecenbot gateway status`, and `reecenbot gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/reecenbot/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `reecenbot models status` (CLI).
...
```bash
curl -fsSL https://reecenbot.ai/install.sh | bash -s -- --install-method git --no-onboard
```
...
5. Run `reecenbot channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Reecenbot scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/reecenbot/reecenbot/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.reecenbot.* still works)
```
...
Ensure the global `reecenbot` CLI is installed and matches the app version:
...
#   ${REECENBOT_CONFIG_PATH:-$HOME/.reecenbot/reecenbot.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
reecenbot gateway --verbose
reecenbot channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/reecenbot/reecenbot-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$REECENBOT_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.reecenbot/logs/...`; profiles use `~/.reecenbot-<profile>/logs/...`)<br />Linux: `journalctl --user -u reecenbot-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Reecenbot Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$REECENBOT_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                            |
| Media cache                       | `$REECENBOT_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                |
| Credentials                       | `$REECENBOT_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                          |
...
reecenbot gateway status --deep

# Is the gateway reachable?
reecenbot health --json
# If it fails, rerun with connection details:
reecenbot health --verbose
...
tail -20 /tmp/reecenbot/reecenbot-*.log
```
...
trash "${REECENBOT_STATE_DIR:-$HOME/.reecenbot}"
reecenbot channels login         # re-pair WhatsApp
reecenbot gateway restart           # or: reecenbot gateway
```
...
1. Check logs first: `/tmp/reecenbot/` (default: `reecenbot-YYYY-MM-DD.log`, or your configured `logging.file`)
