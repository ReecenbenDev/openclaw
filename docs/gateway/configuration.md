---
summary: "All configuration options for ~/.reecenbot/reecenbot.json with examples"
read_when:
...
Reecenbot reads an optional **JSON5** config from `~/.reecenbot/reecenbot.json` (comments + trailing commas allowed).

If the file is missing, Reecenbot uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.reecenbot/workspace`). You usually only need a config to:
...
Reecenbot only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `reecenbot doctor`, `reecenbot logs`, `reecenbot health`, `reecenbot status`, `reecenbot service`, `reecenbot help`).
- Run `reecenbot doctor` to see the exact issues.
- Run `reecenbot doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `reecenbot config set`. Keep a backup of `~/.reecenbot/reecenbot.json`.
...
```bash
reecenbot gateway call config.get --params '{}' # capture payload.hash
reecenbot gateway call config.apply --params '{
...
```bash
reecenbot gateway call config.get --params '{}' # capture payload.hash
reecenbot gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.reecenbot/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@reecenbot", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.reecenbot/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.reecenbot/workspace" }],
}
```
...
{ "$include": "/etc/reecenbot/base.json5" } // absolute
...
```json5
// ~/.reecenbot/clients/mueller/agents.json5
[
...
```json5
// ~/.reecenbot/clients/mueller/broadcast.json5
{
...
Reecenbot reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Reecenbot runs your login shell and imports only the missing expected keys (never overrides).
...
- `REECENBOT_LOAD_SHELL_ENV=1`
- `REECENBOT_SHELL_ENV_TIMEOUT_MS=15000`
...
Reecenbot stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.reecenbot/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.reecenbot/credentials/oauth.json` (or `$REECENBOT_STATE_DIR/credentials/oauth.json`)
...
- `~/.reecenbot/agent/*` (migrated by `reecenbot doctor` into `~/.reecenbot/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `REECENBOT_OAUTH_DIR`
- Agent dir (default agent root override): `REECENBOT_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Reecenbot imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Reecenbot derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/reecenbot/reecenbot-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/reecenbot/reecenbot.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/reecenbot/reecenbot.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.reecenbot/credentials/whatsapp/biz
          // authDir: "~/.reecenbot/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `reecenbot doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@reecenbot", "reecenbot"] } }],
  },
}
```
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@reecenbot"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.reecenbot/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.reecenbot/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.reecenbot/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.reecenbot/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.reecenbot/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.reecenbot/workspace-home" },
      { id: "work", workspace: "~/.reecenbot/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `reecenbot.json`).
...
Reecenbot starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["reecenbot-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
          // guild id (preferred) or slug
          slug: "friends-of-reecenbot",
...
Reecenbot starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `reecenbot plugins install @reecenbot/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Reecenbot starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Reecenbot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Reecenbot
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/reecenbot" } },
}
```
...
When a file exceeds this limit, Reecenbot logs a warning and injects a truncated
...
message envelopes). If unset, Reecenbot uses the host timezone at runtime.
...
`[reecenbot]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[reecenbot]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[reecenbot]"`, Reecenbot will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Reecenbot generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.reecenbot/settings/tts.json",
      elevenlabs: {
...
Reecenbot also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Reecenbot currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `reecenbot configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:reecenbot`: all built-in Reecenbot tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.reecenbot/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.reecenbot/sandboxes",
        docker: {
          image: "reecenbot-sandbox:bookworm-slim",
          containerPrefix: "reecenbot-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "reecenbot-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "reecenbot-sandbox-browser:bookworm-slim",
          containerPrefix: "reecenbot-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Reecenbot uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.reecenbot/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Reecenbot config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Reecenbot writes/merges a `models.json` into
`~/.reecenbot/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Reecenbot uses
...
- Shortcut: `reecenbot onboard --auth-choice opencode-zen`.
...
Shortcut: `reecenbot onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `reecenbot onboard --auth-choice moonshot-api-key`.
...
- Set `KIMI_API_KEY` in the environment or use `reecenbot onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `reecenbot onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `reecenbot onboard --auth-choice minimax-api`.
...
- Override the agent config root with `REECENBOT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.reecenbot/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.reecenbot/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.reecenbot/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Reecenbot stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.reecenbot/skills` (workspace skills
...
from `~/.reecenbot/extensions`, `<workspace>/.reecenbot/extensions`, plus any
...
### `browser` (reecenbot-managed browser)

Reecenbot can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for reecenbot and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Reecenbot.app menubar, or `reecenbot gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      reecenbot: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/reecenbot" }
...
- Examples: `"/ui"`, `"/reecenbot"`, `"/apps/reecenbot"`.
...
- When a connection comes from one of these IPs, Reecenbot uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `reecenbot gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `REECENBOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `REECENBOT_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Reecenbot
...
- Reecenbot.app watches `~/.reecenbot/reecenbot.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.reecenbot/reecenbot.json` (or `REECENBOT_CONFIG_PATH`) and applies changes automatically.
...
- `~/.reecenbot/reecenbot.json` (or `REECENBOT_CONFIG_PATH`)
...
- `REECENBOT_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `reecenbot --dev …` → uses `~/.reecenbot-dev` + shifts ports from base `19001`
- `reecenbot --profile <name> …` → uses `~/.reecenbot-<name>` (port via config/env/flags)
...
```bash
REECENBOT_CONFIG_PATH=~/.reecenbot/a.json \
REECENBOT_STATE_DIR=~/.reecenbot-a \
reecenbot gateway --port 19001
```
...
Gmail helper config (used by `reecenbot webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "reecenbot@gmail.com",
...
- Set `REECENBOT_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Reecenbot defaults `serve.path` to `/` so
...
Default root: `~/.reecenbot/workspace/canvas`  
Default port: `18793` (chosen to avoid the reecenbot browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__reecenbot__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__reecenbot__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
{
  canvasHost: {
    root: "~/.reecenbot/workspace/canvas",
    port: 18793,
    liveReload: true,
  },
}
...
- env: `REECENBOT_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.reecenbot/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.reecenbot/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.reecenbot/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_reecenbot-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `reecenbot` (advertises `reecenbot.local`). Override with `REECENBOT_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_reecenbot-gw._tcp` under `~/.reecenbot/dns/` using the configured discovery domain (example: `reecenbot.internal.`).
