---
title: Lobster
summary: "Typed workflow runtime for Reecenbot with resumable approval gates."
description: Typed workflow runtime for Reecenbot — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets Reecenbot run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Reecenbot runs one Lobster tool call and gets a structured result.
...
Reecenbot launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | reecenbot.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
reecenbot.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Reecenbot tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Reecenbot Gateway (see the [Lobster repo](https://github.com/reecenbot/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Reecenbot keeps core tools enabled. To restrict core
...
→ reecenbot calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ reecenbot calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Reecenbot tools that do.
