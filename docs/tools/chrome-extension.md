---
summary: "Chrome extension: let Reecenbot drive your existing Chrome tab"
read_when:
...
The Reecenbot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate reecenbot-managed Chrome profile.
...
Reecenbot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Reecenbot release (npm package) as static files. There is no separate “build” step.

After upgrading Reecenbot:

- Re-run `reecenbot browser extension install` to refresh the installed files under your Reecenbot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Reecenbot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Reecenbot to control.
...
- `ON`: attached; Reecenbot can drive that tab.
...
Debugging: `reecenbot sandbox explain`
...
`reecenbot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `reecenbot browser extension install` first to copy the extension to a stable location under your Reecenbot state directory.
...
- **This is not isolated** like the dedicated reecenbot-managed profile.
