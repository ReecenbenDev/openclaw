...
- **gateway host** → `reecenbot` process on the gateway machine
...
`~/.reecenbot/exec-approvals.json` directly.

CLI: `reecenbot approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
