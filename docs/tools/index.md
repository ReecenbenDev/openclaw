---
summary: "Agent tool surface for Reecenbot (browser, canvas, nodes, message, cron) replacing legacy `reecenbot-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `reecenbot-*` skills
title: "Tools"
---

# Tools (Reecenbot)

Reecenbot exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `reecenbot-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `reecenbot.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Reecenbot logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`reecenbot node run`).
...
- Requires a Brave API key (recommended: `reecenbot configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Reecenbot-managed browser.
...
- Quick smoke: `reecenbot nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `reecenbot gateway` restart in-place)
