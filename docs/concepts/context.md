...
read_when:
  - You want to understand what “context” means in Reecenbot
...
“Context” is **everything Reecenbot sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (Reecenbot-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How Reecenbot builds the system prompt

The system prompt is **Reecenbot-owned** and rebuilt each run. It includes:
...
By default, Reecenbot injects a fixed set of workspace files (if present):
