...
- `/usage cost` in chats: local cost summary aggregated from Reecenbot session logs.
- CLI: `reecenbot status --usage` prints a full per-provider breakdown.
- CLI: `reecenbot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
