---
summary: "First-run onboarding flow for Reecenbot (macOS app)"
read_when:
...
- Writes credentials to `~/.reecenbot/credentials/oauth.json`
...
The app can install the global `reecenbot` CLI via npm/pnpm so terminal
...
On the first agent run, Reecenbot bootstraps a workspace (default `~/.reecenbot/workspace`):
...
```bash
reecenbot webhooks gmail setup --account you@gmail.com
```
...
- `~/.reecenbot/credentials/oauth.json`
