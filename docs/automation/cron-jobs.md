...
- Jobs persist under `~/.reecenbot/cron/` so restarts don’t lose schedules.
...
reecenbot cron list
reecenbot cron run <job-id> --force
reecenbot cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.reecenbot/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `reecenbot cron add/edit` or the cron
...
- Job store: `~/.reecenbot/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.reecenbot/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.reecenbot/cron/jobs.json",
...
reecenbot cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
reecenbot cron edit <jobId> --agent ops
reecenbot cron edit <jobId> --clear-agent
```
...
```bash
reecenbot system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`reecenbot system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `REECENBOT_SKIP_CRON`.
