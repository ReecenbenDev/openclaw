...
Deploy Reecenbot on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/reecenbot/reecenbot"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run Reecenbot:
...
The web dashboard is available at `https://<your-service>.onrender.com/reecenbot`.
...
If you use the original Reecenbot repository, Render will not auto-deploy your Reecenbot. To update it, run a manual Blueprint sync from the dashboard.
...
For Reecenbot, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Reecenbot host.
