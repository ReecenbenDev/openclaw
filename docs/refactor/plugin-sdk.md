...
- Publish as `reecenbot/plugin-sdk` (or export from core under `reecenbot/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `ReecenbotPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ReecenbotConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ReecenbotConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ReecenbotConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ReecenbotConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ReecenbotConfig): string;
  };
};
```
...
- Introduce `reecenbot/plugin-sdk`.
- Add `api.runtime` to `ReecenbotPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `reecenbotRuntime: ">=2026.2.0"`).
