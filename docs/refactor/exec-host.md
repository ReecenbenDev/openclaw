...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.reecenbot/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.reecenbot/node.json` (node id + pairing token).
...
Path: `~/.reecenbot/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.reecenbot/exec-approvals.sock",
...
- Unix socket at `~/.reecenbot/exec-approvals.sock` (0600).
