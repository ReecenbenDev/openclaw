...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `reecenbot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `reecenbot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=reecenbot-sbx-"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${REECENBOT_STATE_DIR:-$HOME/.reecenbot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
