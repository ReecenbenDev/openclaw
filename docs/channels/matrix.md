...
Matrix is an open, decentralized messaging protocol. Reecenbot connects as a Matrix **user**
...
```bash
reecenbot plugins install @reecenbot/matrix
```

Local checkout (when running from a git repo):

```bash
reecenbot plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `reecenbot plugins install @reecenbot/matrix`
   - From a local checkout: `reecenbot plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Reecenbot calls the same
     login endpoint, stores the access token in `~/.reecenbot/credentials/matrix/credentials.json`,
...
- On first connection, Reecenbot requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Reecenbot logs a warning.
...
`~/.reecenbot/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `reecenbot pairing list matrix`
  - `reecenbot pairing approve matrix <CODE>`
...
- On startup, Reecenbot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
