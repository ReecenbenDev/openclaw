...
```bash
reecenbot plugins install @reecenbot/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
reecenbot plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Reecenbot" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `reecenbot pairing list nextcloud-talk`
  - `reecenbot pairing approve nextcloud-talk <CODE>`
