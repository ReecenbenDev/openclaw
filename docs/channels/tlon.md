...
Tlon is a decentralized messenger built on Urbit. Reecenbot connects to your Urbit ship and can
...
```bash
reecenbot plugins install @reecenbot/tlon
```

Local checkout (when running from a git repo):

```bash
reecenbot plugins install ./extensions/tlon
```
...
Use these with `reecenbot message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Reecenbot replies in-thread.
