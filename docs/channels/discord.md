...
- Talk to Reecenbot via Discord DMs or guild channels.
...
3. Configure Reecenbot with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `reecenbot pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running Reecenbot in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents Reecenbot needs
...
Discord uses numeric ids everywhere; Reecenbot config prefers ids.
...
- First: run `reecenbot doctor` and `reecenbot channels status --probe` (actionable warnings + quick audits).
...
When enabled, Reecenbot uses the member identity for allowlists and labels the
...
- On startup, Reecenbot resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:

- The registered commands mirror Reecenbot’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; Reecenbot enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`reecenbot gateway --force`) after confirming no other processes own the Discord session.
