...
read_when:
  - Setting up Twitch chat integration for Reecenbot
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Reecenbot connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
reecenbot plugins install @reecenbot/twitch
```

Local checkout (when running from a git repo):

```bash
reecenbot plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `REECENBOT_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "reecenbot", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use REECENBOT_TWITCH_ACCESS_TOKEN env var)
...
```bash
REECENBOT_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
