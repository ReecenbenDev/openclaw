...
service via the external `reecenbot` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`REECENBOT_PROFILE`; legacy `com.reecenbot.*` is supported).
