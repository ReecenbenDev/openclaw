---
summary: "Reecenbot on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Reecenbot on a Raspberry Pi
  - Running Reecenbot on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Reecenbot on Raspberry Pi

## Goal

Run a persistent, always-on Reecenbot Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Reecenbot features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled reecenbot

# Enable if not
sudo systemctl enable reecenbot

# Start on boot
sudo systemctl start reecenbot
```
...
```bash
# Check logs
journalctl -u reecenbot --no-pager -n 100

# Common fix: rebuild
cd ~/reecenbot  # if using hackable install
npm run build
sudo systemctl restart reecenbot
```
