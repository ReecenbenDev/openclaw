...
read_when:
  - Designing workspace memory (~/.reecenbot/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Reecenbot integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.reecenbot/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.reecenbot/workspace` as canonical human-readable memory.
...
```
~/.reecenbot/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Reecenbot**, but keep a separable core library.

### Why integrate into Reecenbot?

- Reecenbot already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `reecenbot memory recall "…" --k 25 --since 30d`
  - `reecenbot memory reflect --since 7d`
...
Pragmatic take for `~/.reecenbot/workspace`:
