...
Script: [`scripts/bench-model.ts`](https://github.com/reecenbot/reecenbot/blob/main/scripts/bench-model.ts)
...
This script drives the interactive wizard via a pseudo-tty, verifies config/workspace/session files, then starts the gateway and runs `reecenbot health`.
