...
Reecenbot features that can generate provider usage or paid API calls.
...
- `reecenbot status --usage` and `reecenbot channels list` show provider **usage windows**
...
Reecenbot can pick up credentials from:
...
- `reecenbot status --usage`
- `reecenbot models status --json`
...
`reecenbot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
