---
summary: "Use Synthetic's Anthropic-compatible API in Reecenbot"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Reecenbot registers it as the
...
```bash
reecenbot onboard --auth-choice synthetic-api-key
```
...
Note: Reecenbot's Anthropic client appends `/v1` to the base URL, so use
