---
summary: "Use Venice AI privacy-focused models in Reecenbot"
read_when:
  - You want privacy-focused inference in Reecenbot
...
```bash
reecenbot onboard --auth-choice venice-api-key
```
...
```bash
reecenbot chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Reecenbot shows all available Venice models. Pick based on your needs:
...
```bash
reecenbot models set venice/claude-opus-45
reecenbot models set venice/llama-3.3-70b
```

List all available models:

```bash
reecenbot models list | grep venice
```

## Configure via `reecenbot configure`

1. Run `reecenbot configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Reecenbot automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
reecenbot chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
reecenbot chat --model venice/claude-opus-45

# Use uncensored model
reecenbot chat --model venice/venice-uncensored

# Use vision model with image
reecenbot chat --model venice/qwen3-vl-235b-a22b

# Use coding model
reecenbot chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `reecenbot models list` to see currently available models. Some models may be temporarily offline.
