---
summary: "Use MiniMax M2.1 in Reecenbot"
read_when:
  - You want MiniMax models in Reecenbot
...
```bash
reecenbot plugins enable minimax-portal-auth  # skip if already loaded.
reecenbot gateway restart  # restart if gateway is already running
reecenbot onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/reecenbot/reecenbot/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `reecenbot.json`:
...
- Use `reecenbot models list` and `reecenbot models set minimax/MiniMax-M2.1` to switch.
...
- Running `reecenbot configure` and selecting **MiniMax M2.1**, or
