...
This guide summarizes a sane workflow for working on the pi integration in Reecenbot.
...
- Trigger the agent directly:
  - `pnpm reecenbot agent --message "Hello" --thinking low`
...
State lives under the Reecenbot state directory. Default is `~/.reecenbot`. If `REECENBOT_STATE_DIR` is set, use that directory instead.

To reset everything:

- `reecenbot.json` for config
...
- https://docs.reecenbot.ai/testing
- https://docs.reecenbot.ai/start/getting-started
