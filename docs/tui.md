...
```bash
reecenbot tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `reecenbot tui --deliver`
...
- Check the Gateway logs: `reecenbot logs --follow`.
- Confirm the agent can run: `reecenbot status` and `reecenbot models status`.
...
- No agents in picker: check `reecenbot agents list` and your routing config.
