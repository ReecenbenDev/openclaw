...
export async function handleWhatsAppStart(host: ReecenbotApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: ReecenbotApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: ReecenbotApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: ReecenbotApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: ReecenbotApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: ReecenbotApp): string {
...
export function handleNostrProfileCancel(host: ReecenbotApp) {
...
export function handleNostrProfileToggleAdvanced(host: ReecenbotApp) {
  const state = host.nostrProfileFormState;
  if (!state) return;
  host.nostrProfileFormState = {
    ...state,
...
export async function handleNostrProfileSave(host: ReecenbotApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: ReecenbotApp) {
  const state = host.nostrProfileFormState;
