...
import Foundation
import Testing
@testable import ReecenbotChatUI
...
private final class TestChatTransport: @unchecked Sendable, ReecenbotChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ReecenbotChatHistoryPayload]
    private let sessionsResponses: [ReecenbotChatSessionsListResponse]

    private let stream: AsyncStream<ReecenbotChatTransportEvent>
    private let continuation: AsyncStream<ReecenbotChatTransportEvent>.Continuation

    init(
        historyResponses: [ReecenbotChatHistoryPayload],
        sessionsResponses: [ReecenbotChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<ReecenbotChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> ReecenbotChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ReecenbotChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [ReecenbotChatAttachmentPayload]) async throws -> ReecenbotChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return ReecenbotChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ReecenbotChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ReecenbotChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ReecenbotChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = ReecenbotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = ReecenbotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { ReecenbotChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = ReecenbotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = ReecenbotChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = ReecenbotChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { ReecenbotChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
