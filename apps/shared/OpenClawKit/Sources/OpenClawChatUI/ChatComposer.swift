...
@MainActor
struct ReecenbotChatComposer: View {
    @Bindable var viewModel: ReecenbotChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(ReecenbotChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(ReecenbotChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ReecenbotPendingAttachment.id)
                { (att: ReecenbotPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ReecenbotPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(ReecenbotChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ReecenbotChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(ReecenbotChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(ReecenbotChatTheme.subtleCard)
        .clipShape(Capsule())
    }
