...
private let chatUILogger = Logger(subsystem: "ai.reecenbot", category: "ReecenbotChatUI")

@MainActor
@Observable
public final class ReecenbotChatViewModel {
    public private(set) var messages: [ReecenbotChatMessage] = []
    ...
    public var attachments: [ReecenbotPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ReecenbotChatPendingToolCall] = []
    public private(set) var sessions: [ReecenbotChatSessionEntry] = []
    private let transport: any ReecenbotChatTransport
    ...
    private var pendingToolCallsById: [String: ReecenbotChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any ReecenbotChatTransport) {
    ...
    public var sessionChoices: [ReecenbotChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [ReecenbotChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [ReecenbotChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: ReecenbotPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ReecenbotChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ReecenbotChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [ReecenbotChatMessage]) -> [ReecenbotChatMessage] {
        var result: [ReecenbotChatMessage] = []
    ...
    private static func dedupeKey(for message: ReecenbotChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ReecenbotChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> ReecenbotChatAttachmentPayload in
            ReecenbotChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> ReecenbotChatSessionEntry {
        ReecenbotChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: ReecenbotChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: ReecenbotChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: ReecenbotAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ReecenbotChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> ReecenbotPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
