...
enum ReecenbotLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = ReecenbotOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ReecenbotFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ReecenbotLogging.bootstrapIfNeeded()
        let label = ReecenbotLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ReecenbotOSLogHandler: LogHandler {
...
struct ReecenbotFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ReecenbotLogging.parseLabel(self.label)
