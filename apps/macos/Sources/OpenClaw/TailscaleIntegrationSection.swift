...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.reecenbot/reecenbot.json. Prefer OPENCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.reecenbot/reecenbot.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.reecenbot/reecenbot.json. Restart the gateway to apply."
        }
