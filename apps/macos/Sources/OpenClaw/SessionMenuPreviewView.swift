...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.reecenbot", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> ReecenbotSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: ReecenbotSessionPreviewEntry,
    ...
    private static func cache(payload: ReecenbotSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: ReecenbotChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [ReecenbotKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [ReecenbotKit.AnyCodable]) -> [ReecenbotChatMessage] {
    ...
    private static func previewText(for message: ReecenbotChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: ReecenbotChatMessage) -> Bool {
    ...
    private static func toolNames(for message: ReecenbotChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: ReecenbotChatMessage) -> String? {
