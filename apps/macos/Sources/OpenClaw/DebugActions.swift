...
enum DebugActions {
    ...
    static func openConfigFolder() {
        let url = ReecenbotPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Reecenbot", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = ReecenbotPaths.configURL
        guard
