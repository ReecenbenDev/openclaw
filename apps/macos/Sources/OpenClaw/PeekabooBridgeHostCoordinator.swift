...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.reecenbot", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: ReecenbotPeekabooBridgeServices?
    private static var openclawSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Reecenbot", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = ReecenbotPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.reecenbotSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.reecenbotSocketPath, privacy: .public)")
    }
...
@MainActor
private final class ReecenbotPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.reecenbot.peekaboo")
