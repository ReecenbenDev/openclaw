...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: ReecenbotOAuthStore.AnthropicOAuthStatus = ReecenbotOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: ReecenbotOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = ReecenbotOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = ReecenbotOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try ReecenbotOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Reecenbot can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: ReecenbotOAuthStore.AnthropicOAuthStatus,
