...
private let webChatSwiftLogger = Logger(subsystem: "ai.reecenbot", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: ReecenbotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ReecenbotChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(ReecenbotChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> ReecenbotChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                ReecenbotGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    ReecenbotChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    ReecenbotAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<ReecenbotChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any ReecenbotChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = ReecenbotChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: ReecenbotChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<ReecenbotChatView>) -> NSViewController
    {
