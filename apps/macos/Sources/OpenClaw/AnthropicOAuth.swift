...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (Reecenbot token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: ReecenbotOAuthStore.AnthropicOAuthStatus = ReecenbotOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.reecenbot", category: "anthropic-oauth")
...
enum ReecenbotOAuthStore {
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "Reecenbot OAuth token file not found"
            case .unreadableFile: "Reecenbot OAuth token file not readable"
            case .invalidJSON: "Reecenbot OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in Reecenbot OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "Reecenbot OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.reecenbotOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".reecenbot", isDirectory: true)
