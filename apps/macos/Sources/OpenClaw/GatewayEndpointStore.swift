...
actor GatewayEndpointStore {
    ...
    private static let staticLogger = Logger(subsystem: "ai.reecenbot", category: "gateway-endpoint")
    ...
    private let logger = Logger(subsystem: "ai.reecenbot", category: "gateway-endpoint")
    ...
    init(deps: Deps = .live) {
        ...
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            root: ReecenbotConfigFile.loadDict(),
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: ReecenbotConfigFile.loadDict())
