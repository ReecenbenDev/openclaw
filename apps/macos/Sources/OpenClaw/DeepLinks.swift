...
private let deepLinkLogger = Logger(subsystem: "ai.reecenbot", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "Reecenbot is paused", message: "Unpause Reecenbot to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Reecenbot agent?", message: body) else { return }
        }
