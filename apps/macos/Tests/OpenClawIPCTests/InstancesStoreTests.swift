...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: ReecenbotProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ReecenbotProtocol.AnyCodable] = [
            "presence": .init([ReecenbotProtocol.AnyCodable(entry)]),
        ]
        let payload = ReecenbotProtocol.AnyCodable(payloadMap)
