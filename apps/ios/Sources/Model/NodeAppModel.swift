...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = ReecenbotCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = ReecenbotCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = ReecenbotCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = ReecenbotCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = ReecenbotCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: ReecenbotLocationMode) async -> Bool {
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case ReecenbotLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case ReecenbotCanvasCommand.present.rawValue,
            ...
            case ReecenbotCanvasA2UICommand.reset.rawValue,
            ...
            case ReecenbotCameraCommand.list.rawValue,
                 ReecenbotCameraCommand.snap.rawValue,
                 ReecenbotCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case ReecenbotScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: ReecenbotNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: ReecenbotNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(ReecenbotLocationGetParams.self, from: req.paramsJSON)) ??
            ReecenbotLocationGetParams()
        ...
        let payload = ReecenbotLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case ReecenbotCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(ReecenbotCanvasPresentParams.self, from: req.paramsJSON)) ??
                ReecenbotCanvasPresentParams()
        ...
        case ReecenbotCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case ReecenbotCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(ReecenbotCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case ReecenbotCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(ReecenbotCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ReecenbotCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(ReecenbotCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case ReecenbotCanvasA2UICommand.reset.rawValue:
            ...
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case ReecenbotCanvasA2UICommand.push.rawValue, ReecenbotCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == ReecenbotCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(ReecenbotCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try ReecenbotCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(ReecenbotCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try ReecenbotCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.reecenbotA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case ReecenbotCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ReecenbotCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(ReecenbotCameraSnapParams.self, from: req.paramsJSON)) ??
                ReecenbotCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case ReecenbotCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(ReecenbotCameraClipParams.self, from: req.paramsJSON)) ??
                ReecenbotCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(ReecenbotScreenRecordParams.self, from: req.paramsJSON)) ??
            ReecenbotScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> ReecenbotLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return ReecenbotLocationMode(rawValue: raw) ?? .off
    }
