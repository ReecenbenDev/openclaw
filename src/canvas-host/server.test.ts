...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("reecenbotCanvasA2UIAction");
    expect(out).toContain("reecenbotSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "reecenbot-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("reecenbotSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__reecenbot__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("reecenbot-a2ui-host");
      expect(html).toContain("reecenbotCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__reecenbot__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("reecenbotA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
