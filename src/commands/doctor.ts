...
export async function doctorCommand(
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("reecenbot configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("reecenbot config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("reecenbot setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("reecenbot doctor --fix")}" to apply changes.`);
  }
