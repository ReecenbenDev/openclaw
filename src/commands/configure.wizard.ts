...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "Reecenbot update wizard" : "Reecenbot configure");
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("reecenbot doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
