...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveReecenbotAgentDir: vi.fn().mockReturnValue("/tmp/reecenbot-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/reecenbot-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/reecenbot-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveReecenbotAgentDir: mocks.resolveReecenbotAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveReecenbotAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/reecenbot-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/reecenbot-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/reecenbot-agent-custom");
