...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveReecenbotAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("reecenbot models auth setup-token")}\` or \`${formatCliCommand("reecenbot configure")}\`.`
          : `Run \`${formatCliCommand("reecenbot configure")}\` or set an API key env var.`;
