...
export async function noteSecurityWarnings(cfg: OpenClawConfig) {
  ...
  const auditHint = `- Run: ${formatCliCommand("reecenbot security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("reecenbot configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("reecenbot config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("reecenbot doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("reecenbot config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
