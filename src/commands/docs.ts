...
const SEARCH_TOOL = "https://docs.openclaw.ai/mcp.SearchReecenbot";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    ...
    if (isRich()) {
      ...
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('reecenbot docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.openclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('reecenbot docs "your query"')}`);
    }
    return;
  }
