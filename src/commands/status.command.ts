...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Reecenbot status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("reecenbot security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("reecenbot security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("reecenbot status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("reecenbot logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("reecenbot status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("reecenbot gateway probe")}`);
  }
}
