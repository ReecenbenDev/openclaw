...
vi.mock("./chrome.js", () => ({
  resolveReecenbotUserDataDir: vi.fn(() => "/tmp/reecenbot-test/reecenbot/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveReecenbotUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "reecenbot-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveReecenbotUserDataDir).mockReturnValue(userDataDir);
