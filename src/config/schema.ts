...
import { ReecenbotSchema } from "./zod-schema.js";
...
export type ConfigSchema = ReturnType<typeof ReecenbotSchema.toJSONSchema>;
...
const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when Reecenbot writes the config.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /reecenbot).",
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `reecenbot plugins update` to locate install sources).",
...
const FIELD_PLACEHOLDERS: Record<string, string> = {
  ...
  "gateway.controlUi.basePath": "/reecenbot",
  "channels.mattermost.baseUrl": "https://chat.example.com",
  "agents.list[].identity.avatar": "avatars/reecenbot.png",
};
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = ReecenbotSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
