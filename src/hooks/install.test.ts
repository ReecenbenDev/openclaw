...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `reecenbot-hook-install-${randomUUID()}`);
...
describe("installHooksFromArchive", () => {
  it("installs hook packs from zip archives", async () => {
    ...
    zip.file(
      ...
      JSON.stringify({
        name: "@reecenbot/zip-hooks",
        version: "0.0.1",
        reecenbot: { hooks: ["./hooks/zip-hook"] },
      }),
    );
    zip.file(
      ...
      [
        ...
        'metadata: {"reecenbot":{"events":["command:new"]}}',
        "---",
        "",
        "# Zip Hook",
      ].join("\n"),
  ...
  it("installs hook packs from tar archives", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@reecenbot/tar-hooks",
        version: "0.0.1",
        reecenbot: { hooks: ["./hooks/tar-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
