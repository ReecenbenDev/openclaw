...
`;
  ...
  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"reecenbot": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"reecenbot": {"events": ["test"]}}');
  });
...
describe("resolveReecenbotMetadata", () => {
  it("extracts reecenbot metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveReecenbotMetadata(frontmatter);
  ...
  it("returns undefined when reecenbot key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        reecenbot: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Reecenbot" },
            { id: "npm", kind: "npm", package: "@reecenbot/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@reecenbot/hook");
  });
...
`;
    ...
    const reecenbot = resolveReecenbotMetadata(frontmatter);
    expect(reecenbot).toBeDefined();
    expect(reecenbot?.emoji).toBe("💾");
    expect(reecenbot?.events).toEqual(["command:new"]);
    expect(reecenbot?.requires?.config).toEqual(["workspace.dir"]);
    expect(reecenbot?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const reecenbot = resolveReecenbotMetadata(frontmatter);
    expect(reecenbot?.emoji).toBe("disk");
    expect(reecenbot?.events).toEqual(["command:new"]);
  });
});
