...
import { scheduleGatewaySigusr1Restart, triggerReecenbotRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Reecenbot in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerReecenbotRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Reecenbot via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
