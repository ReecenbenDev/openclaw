...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `reecenbot-plugins-${randomUUID()}`);
...
describe("discoverReecenbotPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      const { discoverReecenbotPlugins } = await import("./discovery.js");
      return discoverReecenbotPlugins({ workspaceDir });
    });
  ...
  it("loads package extension packs", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "pack",
        reecenbot: { extensions: ["./src/one.ts", "./src/two.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      const { discoverReecenbotPlugins } = await import("./discovery.js");
      return discoverReecenbotPlugins({});
    });
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    fs.writeFileSync(
      path.join(globalExt, "package.json"),
      JSON.stringify({
        name: "@reecenbot/voice-call",
        reecenbot: { extensions: ["./src/index.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@reecenbot/demo-plugin-dir",
        reecenbot: { extensions: ["./index.js"] },
      }),
      "utf-8",
    );
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      const { discoverReecenbotPlugins } = await import("./discovery.js");
      return discoverReecenbotPlugins({ extraPaths: [packDir] });
    });
