...
import { discoverReecenbotPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: ReecenbotPluginDefinition;
  register?: ReecenbotPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as ReecenbotPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as ReecenbotPluginDefinition;
...
export function loadReecenbotPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverReecenbotPlugins({
    workspaceDir: options.workspaceDir,
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: ReecenbotPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as ReecenbotPluginModule;
    } catch (err) {
