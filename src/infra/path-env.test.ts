...
import { ensureReecenbotCliOnPath } from "./path-env.js";

describe("ensureReecenbotCliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling reecenbot exists", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "reecenbot-path-"));
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const cliPath = path.join(appBinDir, "reecenbot");
  ...
  it("prepends mise shims when available", async () => {
    ...
    try {
      const appBinDir = path.join(tmp, "AppBin");
      await fs.mkdir(appBinDir, { recursive: true });
      const appCli = path.join(appBinDir, "reecenbot");
      await fs.writeFile(appCli, "#!/bin/sh\necho ok\n", "utf-8");
      await fs.chmod(appCli, 0o755);

      const localBinDir = path.join(tmp, "node_modules", ".bin");
      await fs.mkdir(localBinDir, { recursive: true });
      const localCli = path.join(localBinDir, "reecenbot");
