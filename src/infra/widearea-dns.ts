...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*reecenbot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "reecenbot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "reecenbot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "reecenbot-gw");
  ...
  records.push(`_reecenbot-gw._tcp IN PTR ${instanceLabel}._reecenbot-gw._tcp`);
  records.push(`${instanceLabel}._reecenbot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._reecenbot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; reecenbot-content-hash: ${contentHash}\n${contentBody}`;
}
