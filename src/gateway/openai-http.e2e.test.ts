    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "reecenbot", messages: [{ role: "user", content: "hi" }] },
          { "x-reecenbot-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-reecenbot-agent-id": "beta",
            "x-reecenbot-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
