...
function renderNodeServiceStartHints(): string[] {
  const base = [
    formatCliCommand("reecenbot node install"),
    formatCliCommand("reecenbot node start"),
  ];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("reecenbot node install --force")}`);
    }
    return;
  }
