...
export function registerDnsCli(program: Command) {
  ...
  dns
    ...
    .option("--domain <domain>", "Wide-area discovery domain (e.g. reecenbot.internal)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by reecenbot dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
