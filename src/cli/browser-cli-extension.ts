...
export async function installChromeExtension(opts?: {
  ...
  if (!hasManifest(src)) {
    throw new Error("Bundled Chrome extension is missing. Reinstall Reecenbot and try again.");
  }
      ...
      defaultRuntime.error(
        info(
          [
            ...
            `- Pin “Reecenbot Browser Relay”, then click it on the tab (badge shows ON)`,
            "",
    ...
    .action(async (_opts, cmd) => {
      ...
      if (!hasManifest(dir)) {
        defaultRuntime.error(
          danger(
            [
              `Chrome extension is not installed. Run: "${formatCliCommand("reecenbot browser extension install")}"`,
