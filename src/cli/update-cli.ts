...
const STEP_LABELS: Record<string, string> = {
  ...
  "reecenbot doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "reecenbot";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const OPENCLAW_REPO_URL = "https://github.com/reecenbot/reecenbot.git";
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("reecenbot@")) {
    return trimmed.slice("reecenbot@".length);
  }
...
async function ensureGitCheckout(params: {
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `OPENCLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set OPENCLAW_GIT_DIR to an empty folder or an reecenbot checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("Reecenbot update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Reecenbot..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Reecenbot install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("reecenbot doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("reecenbot gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g reecenbot@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g reecenbot@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("reecenbot gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("reecenbot doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("reecenbot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("reecenbot gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `reecenbot update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `OPENCLAW_GIT_DIR points at a non-git directory: ${gitDir}. Set OPENCLAW_GIT_DIR to an empty folder or an reecenbot checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Reecenbot to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["reecenbot update", "Update a source checkout (git)"],
        ["reecenbot update --channel beta", "Switch to beta channel (git + npm)"],
        ["reecenbot update --channel dev", "Switch to dev channel (git + npm)"],
        ["reecenbot update --tag beta", "One-off update to a dist-tag or version"],
        ["reecenbot update --no-restart", "Update without restarting the service"],
        ["reecenbot update --json", "Output result as JSON"],
        ["reecenbot update --yes", "Non-interactive (accept downgrade prompts)"],
        ["reecenbot update wizard", "Interactive update wizard"],
        ["reecenbot --update", "Shorthand for reecenbot update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run reecenbot update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.openclaw.ai/cli/update")}`;
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["reecenbot update status", "Show channel + version status."],
          ["reecenbot update status --json", "JSON output."],
          ["reecenbot update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
