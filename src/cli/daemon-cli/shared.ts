...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("reecenbot gateway install", env)}`);
    if (fileLog) {
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  const base = [
    formatCliCommand("reecenbot gateway install", env),
    formatCliCommand("reecenbot gateway", env),
  ];
