...
export async function installCompletion(shell: string, yes: boolean, binName = "reecenbot") {
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c reecenbot -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'reecenbot gateway' and not 'reecenbot other gateway', we need to check the sequence.
