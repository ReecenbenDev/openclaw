...
describe("createReecenbotCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "reecenbot-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from reecenbot read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "reecenbot-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "reecenbot-workspace"),
    ...
    const tools = createReecenbotCodingTools({ sandbox });
