...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("reecenbot-ws-", async (workspaceDir) => {
      await withTempDir("reecenbot-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createReecenbotCodingTools({ workspaceDir });
    ...
    await withTempDir("reecenbot-ws-", async (workspaceDir) => {
      await withTempDir("reecenbot-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello reecenbot");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("reecenbot-ws-", async (workspaceDir) => {
      await withTempDir("reecenbot-override-", async (overrideDir) => {
        const tools = createReecenbotCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("reecenbot-sandbox-", async (sandboxDir) => {
      await withTempDir("reecenbot-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "reecenbot-sbx-test",
        ...
        const tools = createReecenbotCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
