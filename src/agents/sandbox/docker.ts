...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "reecenbot.sandbox=1");
  args.push("--label", `reecenbot.sessionKey=${params.scopeKey}`);
  args.push("--label", `reecenbot.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `reecenbot.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("reecenbot.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`reecenbot sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`reecenbot sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("reecenbot sandbox recreate --all");
}
