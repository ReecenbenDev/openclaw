...
import { __testing, createReecenbotCodingTools } from "./pi-tools.js";
...
describe("createReecenbotCodingTools", () => {
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createReecenbotCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "reecenbot-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "reecenbot-outside.txt");
