...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "reecenbot-embedded-agent-"));
...
const ensureModels = (cfg: OpenClawConfig) => ensureReecenbotModelsJson(cfg, agentDir) as unknown;
