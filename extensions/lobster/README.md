...
- This plugin integrates Lobster with Reecenbot _without core changes_.
...
## Using `openclaw.invoke` (Lobster → Reecenbot tools)

Some Lobster pipelines may include a `openclaw.invoke` step to call back into Reecenbot tools/plugins (for example: `gog` for Google Workspace, `gh` for GitHub, `message.send`, etc.).

For this to work, the Reecenbot Gateway must expose the tool bridge endpoint and the target tool must be allowed by policy:

- Reecenbot provides an HTTP endpoint: `POST /tools/invoke`.
- The request is gated by **gateway auth** (e.g. `Authorization: Bearer …` when token auth is enabled).
- The invoked tool is gated by **tool policy** (global + per-agent + provider + group policy). If the tool is not allowed, Reecenbot returns `404 Tool not available`.
