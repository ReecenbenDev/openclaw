...
type AgentEntry = NonNullable<NonNullable<ReecenbotConfig["agents"]>["list"]>[number];

function listAgents(cfg: ReecenbotConfig): AgentEntry[] {
...
function resolveAgentEntry(cfg: ReecenbotConfig, agentId: string): AgentEntry | undefined {
  const id = normalizeAgentId(agentId);
  return listAgents(cfg).find((entry) => normalizeAgentId(entry.id) === id);
}

export function resolveIdentityName(cfg: ReecenbotConfig, agentId: string): string | undefined {
