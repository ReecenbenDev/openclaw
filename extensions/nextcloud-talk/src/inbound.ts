...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as ReecenbotConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as ReecenbotConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as ReecenbotConfig);
