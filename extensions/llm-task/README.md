...
`openclaw.invoke --each`) without adding new Reecenbot code per workflow.
...
This extension depends on Reecenbot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Reecenbot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.reecenbot/extensions` as a standalone plugin directory.
