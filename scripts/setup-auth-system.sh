#!/bin/bash
# Setup Reecenbot Auth Management System
...
echo "=== Reecenbot Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'reecenbot-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Reecenbot message: Send warning via Reecenbot itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/reecenbot-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/reecenbot-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/reecenbot-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now reecenbot-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status reecenbot-auth-monitor.timer"
echo ""
echo "Quick commands:"
