...
IMAGE_NAME="reecenbot-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/reecenbot-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/reecenbot-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/reecenbot-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/reecenbot-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/reecenbot-e2e-$1.XXXXXX"
  }
