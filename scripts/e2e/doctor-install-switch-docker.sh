...
IMAGE_NAME="reecenbot-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/reecenbot-bin:$PATH"
  mkdir -p /tmp/reecenbot-bin

  cat > /tmp/reecenbot-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/reecenbot-bin/systemctl

  cat > /tmp/reecenbot-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/reecenbot-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/reecenbot"
  npm_entry="/tmp/npm-prefix/lib/node_modules/reecenbot/reecenbot.mjs"
  git_entry="/app/reecenbot.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/reecenbot-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/reecenbot-gateway.service"
    if [ ! -f "$unit_path" ]; then
