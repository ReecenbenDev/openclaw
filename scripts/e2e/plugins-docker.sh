...
IMAGE_NAME="reecenbot-plugins-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  home_dir=$(mktemp -d "/tmp/reecenbot-plugins-e2e.XXXXXX")
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/reecenbot-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@reecenbot/demo-plugin-tgz",
  "version": "0.0.1",
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/reecenbot-plugin-dir.XXXXXX")"
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/reecenbot-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@reecenbot/demo-plugin-file",
  "version": "0.0.1",
  "reecenbot": { "extensions": ["./index.js"] }
}
JSON
