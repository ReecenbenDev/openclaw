...
# Reecenbot Auth Widget for Termux
...
# Check auth status
termux-toast "Checking Reecenbot auth..."

STATUS=$(ssh "$SERVER" '$HOME/reecenbot/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/reecenbot/scripts/claude-auth-status.sh json' 2>&1)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/reecenbot/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|OPENCLAW_EXPIRED|OPENCLAW_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Reecenbot won't work until re-auth"
                ;;
        esac
        ;;
