...
# Build and bundle Reecenbot into a minimal .app we can open.
# Outputs to dist/Reecenbot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Reecenbot.app"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/reecenbot/reecenbot/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Reecenbot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ReecenbotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ReecenbotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Reecenbot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Reecenbot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Reecenbot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Reecenbot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Reecenbot/Resources/Reecenbot.icns" "$APP_ROOT/Contents/Resources/Reecenbot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Reecenbot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ReecenbotKit resources"
OPENCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ReecenbotKit_ReecenbotKit.bundle"
if [ -d "$OPENCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ReecenbotKit_ReecenbotKit.bundle"
  cp -R "$OPENCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ReecenbotKit_ReecenbotKit.bundle"
else
  echo "WARN: ReecenbotKit resource bundle not found at $OPENCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Reecenbot"
killall -q Reecenbot 2>/dev/null || true
