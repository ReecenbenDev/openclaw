...
BASE_IMAGE="${BASE_IMAGE:-reecenbot-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-reecenbot-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=reecenbot.sandbox=1)
NOTE
