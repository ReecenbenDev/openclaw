...
#
# Suggested cron: */30 * * * * /home/admin/reecenbot/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Reecenbot notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., reecenbot-alerts)
...
send_notification() {
    ...
    # Send via Reecenbot if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use reecenbot
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Reecenbot to $NOTIFY_PHONE..."
            reecenbot send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Reecenbot is down. Run: ssh l36 '~/reecenbot/scripts/mobile-reauth.sh'" "urgent"
    exit 1
