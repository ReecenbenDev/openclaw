...
ZIP=${1:?"Usage: $0 Reecenbot-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/reecenbot/reecenbot/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Reecenbot-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/reecenbot/reecenbot/releases/download/v${VERSION}/"}
