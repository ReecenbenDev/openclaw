#!/usr/bin/env bash
# Reset Reecenbot like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.
...
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Reecenbot"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Reecenbot"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.openclaw.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/reecenbot-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${OPENCLAW_RESTART_LOG:-/tmp/reecenbot-restart.log}"
NO_SIGN=0
SIGN=0
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log "  node reecenbot.mjs daemon install --force --runtime node"
      log "  node reecenbot.mjs daemon restart"
      log ""
...
kill_all_reecenbot() {
  for _ in {1..10}; do
    ...
    if ! pgrep -f "${APP_PROCESS_PATTERN}" >/dev/null 2>&1 \
       ...
       && ! pgrep -x "Reecenbot" >/dev/null 2>&1; then
      return 0
    fi
    sleep 0.3
  done
...
log "==> Killing existing Reecenbot instances"
kill_all_reecenbot
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product Reecenbot"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/Reecenbot.app" ]]; then
    APP_BUNDLE="/Applications/Reecenbot.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/Reecenbot.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/Reecenbot.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Reecenbot.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set OPENCLAW_APP_BUNDLE to your installed Reecenbot.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node reecenbot.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node reecenbot.mjs daemon restart"
