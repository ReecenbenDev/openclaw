...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/reecenbot/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Reecenbot Re-Auth" -c "After getting key, run: ssh $SERVER '~/reecenbot/scripts/mobile-reauth.sh'" --id reecenbot-auth
        ;;
    *)
